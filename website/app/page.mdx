...
`hty` wraps any program in a persistent PTY session. Your agent reads the rendered screen, decides what to type, and sends keystrokes — the same loop a human uses, automated.
...
The agent doesn't know the questions in advance — it reads whatever is on screen and responds. If `create-next-app` adds a new prompt next month, the same loop handles it. No flags to memorize. No brittle expect scripts.
