...
A handful of TUIs (btop, lazygit, k9s, tmux copy-mode) are far easier to drive with clicks than with keyboard navigation. `hty send` supports clicks, drags, and scroll events — but only against apps that have opted into mouse mode via the usual `CSI ?1000/1002/1003 h` sequences. The snapshot payload exposes the current state so the agent can check before it clicks:
...
# Drag to select — useful for tmux copy-mode
hty send my-tmux --drag 3 4 3 20
```
