...
//! (`lib/compiler/test_runner.zig`, Zig 0.16.0) except `log` drops logs
//! scoped to `.stream` that are below the err threshold. That scope
//! belongs to `ghostty-vt`, which warns on every escape sequence it
//! doesn't implement (mode 1034 from readline under TERM=xterm-256color,
//! CSI 22/23 t title-stack, CSI m with `?` intermediate from vim). Real
