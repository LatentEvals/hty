//! Project-local test runner. Identical to Zig's default runner
//! (`lib/compiler/test_runner.zig`) except `log` drops logs scoped to
//! `.stream` that are below the err threshold. That scope belongs to
//! `ghostty-vt`, which warns on every escape sequence it doesn't
//! implement (mode 1034 from readline under TERM=xterm-256color, CSI
//! 22/23 t title-stack, CSI m with `?` intermediate from vim). Real
//! programs driven through hty emit these routinely and the noise
//! swamps CI output.
//!
//! Scope filter keeps err-level logs flowing so genuine failures still
//! surface, and doesn't hide logs from any other scope. See #6.
...
pub fn main() void {
    ...
    if (crippled) return mainSimple() catch @panic("test failure\n");
...
/// The single project-local divergence from Zig's default runner:
/// `.stream`-scoped logs below err are dropped. Everything else
/// matches the default behavior so `log_err_count` still captures
/// real failures.
pub fn log(
    ...
    if (scope == .stream and @intFromEnum(message_level) > @intFromEnum(std.log.Level.err)) {
        return;
    }
