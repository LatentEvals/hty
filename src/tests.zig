...
const ServerEntryCtx = struct {
    ...
    /// If non-null, wired through `RunOpts.log_dir` so the server uses the
    /// harness's tmp log dir instead of resolving one from the environment.
...
fn startServerHarness(alloc: std.mem.Allocator, tag: []const u8) !*ServerHarness {
    ...
    defer alloc.free(by_name);
    try std.fs.cwd().makePath(by_name);
    return startServerHarnessImpl(alloc, tag, log_dir, false);
}
...
fn startServerHarnessInLogDir(
    ...
    errdefer alloc.free(log_dir);
    return startServerHarnessImpl(alloc, tag, log_dir, true);
}
...
fn startServerHarnessImpl(
    ...
    ctx.* = .{
        ...
        .log_dir = if (wire_log_dir) harness.log_dir else null,
    };
