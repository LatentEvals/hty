...
/// Plain output strips per-line trailing padding and collapses trailing
/// fill-row runs, same as `hty snapshot`. `lines` (from `--lines N:M`)
/// restricts the output to that row range, applied before the collapse.
